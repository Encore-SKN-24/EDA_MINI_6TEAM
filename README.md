...
## 회고록
- `김현수`</br>
지식적 혹은 능력적 한계에서 야기된 시간 부족 때문에 정말로 원했던 정보에 대한 탐색이 부족했던 것에 대해 아쉬움이 남습니다. 충분히 제공되었던 시간을 체계적으로 이용하지 못했던 것은 더 열심히 공부해야겠다는 결론이 되었습니다. 추가적으로 웹 크롤링을 통해 스포티파이 데일리 차트를 수집하는 과정에서, 드라이버 과부화를 해결하기 위한 대기 시간의 최소값을 찾는 과정에 시간을 많이 들였던 것이 기억에 남습니다. 메모리 효율성 및 과부화 방지의 중요성을 알게 되었습니다.
...
- `진세형`</br>
  단순히 화면이 바뀐다고 동적 크롤링을 적용하기보다, 네트워크 탭을 통해 얻은 url을 활용하여 날씨 데이터를 더 빠르게 수집할 수 있었다는 점에서 의의가 있었습니다. 다만 EDA를 수행할 때 어떤 방식으로 상관관계를 도출하고 어떤 방식으로 시각화 할 것인지 방향성을 잡는 데 어려움이 있었고, 이에 따라 다양한 분석 기법을 사용해보지 못해 아쉽습니다. 분석/시각화 방법론을 미리 정립하여 데이터 분석을 더 많이 수행할 수 있도록 하겠습니다.
